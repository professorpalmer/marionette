...
| History compaction | `harness/compaction_mixin.py` (`CompactionContextMixin`) | Sole conversation-history compressor (`_maybe_compact_history`). Factory residual is `catalog` (extractive handle index plus last-N selected story). Vault retrieve lives in `harness/compaction_vault.py`. Summary and hybrid are Settings opt-ins and never call the live / local pilot; they need a cheaper `HARNESS_COMPACTION_MODEL` or they stay extractive. Distinct from tool-output spill. Lab and paper: https://professorpalmer.github.io/catalog-residual/ |
