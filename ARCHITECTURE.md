...
harness/          the product (principal modules below; mixins compose the session)
  ...
  api/              HTTP route peels (sessions, jobs, sse, streams, wiki, mcp,
                    ...
                    terminals, commands, hooks, checkpoints, git) wired
                    from server.Handler. Handler keeps auth/token gates;
                    route bodies live in api/* with *Services dataclasses.
                    auth re-exports providers' pool/OAuth/Cursor-CLI
                    handlers under an ownership name.
