...
def test_legacy_settled_rows_preserve_archive_without_projecting_settled(tmp_path, archived):
    from harness.sessions import SessionStore

    path = tmp_path / "harness_sessions.json"
    original = json.dumps({
        "sessions": [{
            "id": "legacy1", "title": "Old", "created": 1.0,
            "settled": True, "archived": archived,
            "repo": str(tmp_path), "workspace_root": str(tmp_path),
        }],
        "active": "legacy1",
    })
    ...
    assert rows[0]["archived"] is archived
    assert "settled" not in rows[0]
    assert path.read_text(encoding="utf-8") == original
    store.archive("legacy1", not archived)
    store.flush()
    assert store.list()[0]["archived"] is not archived
    assert "settled" not in store.list()[0]
    assert json.loads(path.read_text(encoding="utf-8"))["sessions"][0]["settled"] is True


def test_removed_settle_endpoint_returns_not_found(tmp_path, monkeypatch):
    from harness.sessions import SessionStore

    httpd, port, srv = _server()
    monkeypatch.setattr(srv, "_sessions", SessionStore(str(tmp_path / "harness_sessions.json")))
    monkeypatch.setattr(srv._cfg, "state_dir", str(tmp_path))
    monkeypatch.setattr(srv._cfg, "repo", "")
    try:
        sid = srv._sessions.create("Regular session")["id"]
        headers = {"Content-Type": "application/json", "X-Harness-Token": srv._TOKEN}
        with pytest.raises(urllib.error.HTTPError) as error:
            _post(port, "/api/sessions/settle", {"session": sid, "settled": True}, headers)
        assert error.value.code == 404
        row = srv._sessions.list()[0]
