...
def test_prompt_cache_warm_requires_recent_read_and_ttl(monkeypatch):
    ...
    warm_session = ConversationalSession(HarnessConfig(driver="anthropic/claude-sonnet-4"))
    warm_session._last_prompt_cache_activity_at = now - 60
    warm_session._last_turn_cache_read_tokens = 5000
    ok, detail = prompt_cache_warm_for_session(warm_session)
    assert ok is True
    assert detail["warm_reason"] == "warm"

    expired = ConversationalSession(HarnessConfig(driver="anthropic/claude-sonnet-4"))
    expired._last_prompt_cache_activity_at = now - 7200
    expired._last_turn_cache_read_tokens = 5000
    ok2, detail2 = prompt_cache_warm_for_session(expired)
    assert ok2 is False
    assert detail2["warm_reason"] == "expired"

    no_read = ConversationalSession(HarnessConfig(driver="anthropic/claude-sonnet-4"))
    no_read._last_prompt_cache_activity_at = now - 60
    no_read._last_turn_cache_read_tokens = 0
