...
if (Test-Path $Py) {
    ...
    if ($LASTEXITCODE -eq 0) { Ok "imports 'puppetmaster'" } else { Bad "cannot import 'puppetmaster' -- run: uv sync --frozen (or: uv pip install --python .venv puppetmaster-ai==1.27.29)" }
} else {
