...
if [ -x "$PY" ]; then
  ...
  if $PY -c "import puppetmaster" 2>/dev/null; then ok "imports 'puppetmaster'"; else bad "cannot import 'puppetmaster' -- run: uv sync --frozen (or: uv pip install --python .venv puppetmaster-ai==1.27.30)"; fi
else
