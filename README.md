...
stdlib-only backend (urllib + sqlite); `puppetmaster-ai==1.27.18` is the one
real dependency the installer puts in the venv.

v0.9.484 recovers bounded transient provider failures, reconciles completed
swarm jobs without a manual refresh, and gives model and reasoning controls
more room while keeping Plan and Autopilot icon-only. It also updates the
Puppetmaster runtime pin to 1.27.18. v0.9.483 hardens audit and cleanup
...
| **Puppetmaster delegation** | `run_swarm`, `run_implement`, and `run_parallel` run bounded workers as durable jobs. Inspect recorded attempts, acceptance evidence, uncertain outcomes, and measured or estimated consumption. Marionette pins `puppetmaster-ai==1.27.18`; the installer and self-update use the same version. |
