...
> Status: v0.9.148, deliberately pre-1.0. Fresh installs now fail closed on degraded swarms, reconcile provider keys and platform routing consistently, expose honest durable artifact provenance, and advertise only usable browser/LSP capabilities; rides puppetmaster-ai==1.21.2.
...
| **Puppetmaster delegation** | run_swarm (read-only analysis), run_implement (edit-capable worktree worker), run_parallel (concurrent waves). Heavy/multi-file work runs as durable, auditable jobs. Requires `puppetmaster-ai==1.21.2` (Claude Opus 5 everyday frontier registry, plus explicit model allowlists / Bedrock invoke health). |
