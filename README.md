...
> Status: v0.9.97, deliberately pre-1.0. Cost provenance now distinguishes provider billing from catalog/default estimates; reliable live transcript containment/replay, vetted privately before any wider release.
...
| **Puppetmaster delegation** | run_swarm (read-only analysis), run_implement (edit-capable worktree worker), run_parallel (concurrent waves). Heavy/multi-file work runs as durable, auditable jobs. Requires `puppetmaster-ai==1.20.1` (includes `prewalk` plan-then-cheap). |
