...
> Status: v0.9.164, deliberately pre-1.0. Swarm dispatches can target a separate subject checkout for read-only audits, and every completed run carries deterministic current-job evidence with structured acceptance-criterion verification (rides puppetmaster-ai==1.21.9).
