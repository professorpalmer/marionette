...
> Status: v0.9.132, deliberately pre-1.0. Wedged-turn reap keeps `_busy_since` until release succeeds; send-stale recovers even if on_interrupt raises; analysis structured gate fails closed on exception (no early run_auto halt on mid-thought prose). Rides puppetmaster-ai==1.20.10.
...
| **Puppetmaster delegation** | run_swarm (read-only analysis), run_implement (edit-capable worktree worker), run_parallel (concurrent waves). Heavy/multi-file work runs as durable, auditable jobs. Requires `puppetmaster-ai==1.20.10` (includes explicit model allowlists, bounded reroute, and verified Bedrock invoke health). |
