...
> Status: v0.9.410, deliberately pre-1.0. Rides puppetmaster-ai==1.22.45. A provider drop (`finish_reason=error`) emits dirty `assistant_done` immediately; Waiting-on-model chrome uses the current quiet-phase clock, not the whole-turn timer. Mid-stream GLM tool markup stays out of persisted assistant prose. Settings, `/api/restart`, and diagnostic recovery quit and reopen the app so backend and UI boot together; unexpected backend death still respawns Python only. Source-update npm ci rehydrates Electron 44 before relaunch; TODO markers spin only when a live job correlates; incomplete-reply chips stay on their owning turn after hydrate. Skill catalog stays in the frozen prefix (bodies retrieve per turn); git spawns neutralize repo hooks; tools[] freeze across sends with a `/reload-mcp` hatch; cron keeps a continuity digest and can suppress failure notices; compaction fail-until persists and idle ungrown transcripts skip auto-compact; vault retrieve is fail-closed; analysis/QA roles cannot write. Pi TUI/pilot package (not a worker adapter).
