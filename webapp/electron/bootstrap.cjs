...
function installIdentity(dir, target) {
  ...
  return { schema: 1, mode: target.mode, repo: target.repo,
    ...
    puppetmaster: process.env.MARIONETTE_PUPPETMASTER_SPEC || "puppetmaster-ai==1.27.24" };
}
...
async function provisionPython(dest, onProgress) {
  ...
  const spec = process.env.MARIONETTE_PUPPETMASTER_SPEC || "puppetmaster-ai==1.27.24";
