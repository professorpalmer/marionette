...
async function python(action, args = {}, sessionId = "smoke") {
  const code = "import json,os,sys; from harness import desktop_browser as d,browser as b; d.configure(int(os.environ['SMOKE_PORT']),os.environ['SMOKE_TOKEN']); args=json.loads(sys.argv[2]); print(getattr(b,'browser_'+sys.argv[1])(**args,session_id=sys.argv[3]))";
