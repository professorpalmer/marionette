...
test("runtime parity: a venv already at the checkout pin is left alone", async () => {
  const fake = fakeRun({ show: showOutput("Version: 1.21.14\nLocation: /venv/site-packages") });
  const res = await ensure({ run: fake.run, resolvePin: pinAt("1.21.14") });
  assert.equal(res.status, "current");
  assert.equal(res.version, "1.21.14");
...
test("runtime parity: without uv it falls back to the venv's own pip", async () => {
  ...
  assert.equal(res.status, "upgraded");
  assert.deepEqual(fake.installs(), [
    `${PYTHON} -m pip install --upgrade puppetmaster-ai==1.21.14 --quiet`,
  ]);
});

test("runtime parity: a missing venv interpreter is stale, not silently current", async () => {
  const fake = fakeRun({ show: showOutput("Version: 1.21.14") });
  const res = await ensure({ run: fake.run, resolvePin: pinAt("1.21.14"), exists: () => false });
  assert.equal(res.status, "stale");
...
test("runtimeParityFields: only a stale runtime annotates the update-check payload", async () => {
  ...
  const stale = await ensure({ run: fake.run, resolvePin: pinAt("1.21.14") });
  ...
  assert.equal(fields.runtimeWant, "1.21.14");
  assert.match(fields.runtimeNote, /1\.21\.14/);

  const ok = fakeRun({ show: showOutput("Version: 1.21.14") });
  const current = await ensure({ run: ok.run, resolvePin: pinAt("1.21.14") });
