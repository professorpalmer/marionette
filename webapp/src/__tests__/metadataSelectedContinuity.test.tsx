...
async function fixture() {
    ...
    requestJSON: async (method: string, path: string, body: unknown) => {
      ...
      try {
        ...
        if (url.pathname === '/api/endpoint') value = handshake;
          ...
          status = detailStatus;
          value = { ...wire.detail, local_ref: captured.local_ref, summary: omitSummary ? undefined : captured, rows: [], total: 0,
            page: { outcome: detailOutcome, revision: captured.revision, checkpoint: captured.revision, scanned: 0, next_cursor: null } };
  ...
  const finish = () => { detailSummary = { ...selected, revision: selected.revision + 1, lifecycle: 'cancelled', receipts: { ...selected.receipts, terminal: true } }; };
...
it('reconciles accepted Stop copy when a later inspection observes cancellation', async () => {
  const f = await fixture(); await f.open();
  fireEvent.click(screen.getByRole('button', { name: 'Request native stop' }));
  await screen.findByText(/Stop request accepted; awaiting lifecycle observation/);
  f.finish();
  fireEvent.click(screen.getByRole('button', { name: 'Inspect actions' }));
  await screen.findByText(/Lifecycle: cancelled/);
  expect(screen.getByText('Stop request accepted; observed lifecycle: cancelled.')).toBeVisible();
  expect(screen.queryByText(/awaiting lifecycle observation/)).not.toBeInTheDocument();
  expect(f.stops).toHaveLength(1);
});

it('replaces an unavailable empty lane with the latest unavailable observation', async () => {
  const f = await fixture(); f.failDetail('unavailable');
  fireEvent.click(screen.getByRole('button', { name: 'Command · running' }));
  fireEvent.click(screen.getByRole('button', { name: 'Inspect actions' }));
  await screen.findByText(/actions: unavailable/);
  f.finish();
  fireEvent.click(screen.getByRole('button', { name: 'Inspect actions' }));
  await screen.findByText(/Lifecycle: cancelled/);
  const observation = f.store.getSnapshot().localDetail?.observation;
  expect(observation?.page.revision).toBe(observation?.summary?.revision);
  expect(screen.queryByText(/Retained actions rows are stale/)).not.toBeInTheDocument();
});
