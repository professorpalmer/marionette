...
it.each([360, 640, 1024])("keeps both requested rails in the desktop budget at %ipx", width => {
  ...
  expect(result.leftW + result.rightW + layoutChrome(true, true)).toBeLessThanOrEqual(width);
});
