...
LIVE SELF-EDITING (how your changes take effect): When "Live Self-Editing" is enabled (Settings > Advanced), the backend runs from the editable source checkout and the React UI is served from a Vite dev server, so your edits to this repo ARE the running code. Edits to the React UI (`webapp/src/**`) hot-reload INSTANTLY (HMR) -- no rebuild, no restart. Edits to backend Python (`harness/**`) require restarting the backend to load (Settings > Advanced > "Restart backend", or POST `/api/restart`); the conversation auto-resumes across the restart, so an in-flight turn is not lost. Never claim a fundamental/backend change is "live" without a restart -- state plainly that a restart is required and, when appropriate, trigger it. If Live Self-Editing is OFF, the app runs a frozen bundled backend + prebuilt UI, and self-edits only land through a full self-update (git pull + rebuild), not a restart.
