"""Stateful scrubber for inline thinking tags in streamed assistant text.

Lifted from Hermes-agent ``agent/think_scrubber.py`` (MIT) into Marionette so
OpenAI-compat stream deltas do not leak think/reasoning blocks when tags split
across chunks. Wire via feed() per delta and flush() at end of stream; keep
strip_think_blocks for final assembled text.
"""
